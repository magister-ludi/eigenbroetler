...
ExportDialog::ExportDialog(QWidget *p, DisplayInfo::ComplexComponent cmp):
    QDialog(p)
{
    ui.setupUi(this);
    QString const& left = cmp == DisplayInfo::REAL ? tr("&Real") : tr("&Magnitude");
    QString const& right = cmp == DisplayInfo::REAL ? tr("&Imaginary") : tr("&Phase");
    ...
    left_name = QString("ExportDialog/") + (cmp == DisplayInfo::REAL ? "real" : "magnitude");
    right_name = QString("ExportDialog/") + (cmp == DisplayInfo::REAL ? "imaginary" : "phase");
    both_name = QString("ExportDialog/") + (cmp == DisplayInfo::REAL ? "real-imaginary" : "magnitude-phase");
    QSettings settings(EigenbroetlerWindow::app_owner, EigenbroetlerWindow::app_name);
